...
-------
select sd.month, sum(sd.amount*p.price) as month_total from sale_detail sd, product p group by sd.month where sd.product_id=p.id into table p3
-------
select p.* from product p where p.id not in (select sd.product_id from sale_detail sd group by sd.product_id) into table p4
-------
select p.id, sd.month, sum(sd.amount * p.price) as month_total from sale_detail sd ,product p group by p.id, sd.month where sd.product_id=p.id into table p5
-------
select sd.sales_id ,sd.month , sum(sd.amount*p.price) as month_total from sale_detail sd, product p group by sd.sales_id, sd.month where sd.product_id=p.id into table p1
